...
function Appointment() {
  ...
  const fetchAppointments = async () => {
    try {
      const res = await axios.get(
        "https://visitor-pass-system-2.onrender.com/api/appointments",
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const approveAppointment = async (id) => {
    try {
      await axios.put(
        `https://visitor-pass-system-2.onrender.com/api/appointments/${id}/approve`,
        { status: "approved" },
        {
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  const downloadPDF = async (passId) => {
    try {
      const res = await axios.get(
        `https://visitor-pass-system-2.onrender.com/api/pass/pdf/${passId}`,
        {
          headers: {
